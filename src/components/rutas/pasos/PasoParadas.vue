<template>
  <div class="space-y-4">
    ...
    <!-- Controles Globales -->
    <div v-if="form.paradas.length > 0" class="flex justify-end px-1">
      <button 
        @click="colapsarTodas" 
    ...
    <div class="space-y-3 pt-1">
      ...
      >
        ...
        <div class="flex items-center justify-between gap-2">
          ...
          <div 
            class="flex items-center gap-2 flex-1 cursor-pointer select-none group"
            @click="toggleColapso(parada)"
          >
            <component 
              :is="parada._colapsado ? ChevronRightIcon : ChevronDownIcon" 
              class="w-4 h-4 text-gray-400 group-hover:text-brand transition-colors"
            />
            <span class="w-5 h-5 rounded-full bg-brand text-white text-[10px] flex items-center justify-center font-extrabold shadow-sm shrink-0">
              {{ parada.orden_parada }}
            </span>
            <div class="flex flex-col">
              <h4 class="font-bold text-xs text-gray-800 uppercase tracking-wide truncate">
                {{ parada.nombre_parada || 'Parada Sin Nombre' }}
              </h4>
              <span v-if="parada._colapsado" class="text-[10px] text-gray-500 font-mono">
                {{ parada.latitud || 'Sin lat' }}, {{ parada.longitud || 'Sin lon' }}
              </span>
            </div>
          </div>

          <!-- Lado Derecho: Controles (Subir, Bajar, Eliminar) -->
          <div class="flex items-center gap-1 shrink-0">
            <button 
              @click="moverParada(index, -1)" 
              :disabled="index === 0"
              title="Mover arriba"
              class="p-1.5 text-gray-400 hover:text-brand hover:bg-brand/10 rounded-lg transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:cursor-not-allowed"
            >
        ...
        <!-- CUERPO DE LA PARADA (Ocultable) -->
        <div v-show="!parada._colapsado" class="space-y-3.5 mt-4 pt-4 border-t border-gray-100">
          ...
          <div v-if="errores?.paradas?.[index]?.distancia_trazado" class="text-[10px] font-bold text-red-600 bg-red-100 p-2 rounded-lg">
...
const colapsarTodas = () => {
  const algunaAbierta = form.value.paradas.some(p => !p._colapsado)
  form.value.paradas.forEach(p => { p._colapsado = algunaAbierta })
}
...
const moverParada = (index, direccion) => {
  ...
  if (direccion === 1 && index === form.value.paradas.length - 1) return

  const nuevasParadas = [...form.value.paradas]
  
 
  const temp = nuevasParadas[index]
  nuevasParadas[index] = nuevasParadas[index + direccion]
  nuevasParadas[index + direccion] = temp
  
  nuevasParadas.forEach((p, i) => { p.orden_parada = i + 1 })
  
  form.value.paradas = nuevasParadas
